...
Although not shown, the WiFi experiments follow the same trends, with differences in absolute values due to system parameters. In particular, WiFi exhibits lower tag-bits per message compared to 5G due to shorter tag lengths (32 vs. 64 bytes) and smaller payload sizes, as summarized in Table~\ref{tab:setting}. These differences do not affect the relative behavior of the schemes.

Furthermore, by observing the trend of the OptiMAC configurations (blue points) in the goodput analysis (Figure~\ref{fig:goodput5G}), we identify a characteristic curve indicating an optimal operating region where goodput is maximized while maintaining required security levels (Goal G1). Selecting configurations near this peak enables an effective balance between security and efficiency, ensuring high performance under varying attack conditions.
...
Our prototype is implemented in Python at the application layer and is not optimized for performance. Nevertheless, OptiMAC supports real-time streaming at practical frame rates. A production-level implementation (e.g., in C/C++ or at the data link layer) is expected to further reduce overhead and latency. Overall, these results demonstrate that OptiMAC provides a practical balance between security, communication overhead, and computational efficiency, with increasing advantages under adversarial conditions.